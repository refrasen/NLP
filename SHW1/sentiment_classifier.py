...
def fancy_featurize(text):
    ...
    features.update(bitrigrams(text))
    # features["average number of negative AFINN words"] = AFINN_values(text)[0]
    # features["average number of positive AFINN words"] = AFINN_values(text)[1]
    # features["average number of negative SENTI words"] = SENTI_values(text)[0]
    # features["average number of positive SENTI words"] = SENTI_values(text)[1]
    features["average AFINN"] = avg_AFINN(text)
    features["average SENTI"] = avg_SENTI(text)
    return features
...
def negated_words(text):
    ...
    words = text.split()
    for word in words:
        stripped = word.strip(punct).lower()
        ...
        if any(char in word for char in punct):
            negation = False
    return r
...
def AFINN_values(text):
    ...
    if total == 0:
        total = 1
    return sum_neg/total, sum_pos/total


with open("C:\\Users\\Renee\\PycharmProjects\\NLP\\SHW1\\SentiWords_1.1.txt") as f:
    lines = f.readlines()
    SentiWords = {}
    for line in lines:
        key,val = line.split(":")
        SentiWords[key.lower()] = float(val.strip())
    f.close()

def SENTI_values(text):
    punct = "?.,!:;"
    words = text.split()
    total = 0
    sum_pos = 0
...
L2_REGULARIZATION_STRENGTH = {"dumb_featurize": 1, "fancy_featurize": .01}
...
class SentimentClassifier:
    ...
    def evaluate(self, trainX, trainY, devX, devY):
        ...
        print("Method: %s, Features: %s, Train accuracy: %.3f, Dev accuracy: %.3f" % (
            self.feature_method.__name__, F, training_accuracy, development_accuracy))
